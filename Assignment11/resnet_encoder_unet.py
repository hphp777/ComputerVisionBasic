...
class UNetWithResnet50Encoder(nn.Module):
    def __init__(self, n_classes=22):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(stride=2, kernel_size=7, in_channels=3, padding=3, out_channels=64), # Code overlaps with previous assignments
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, with_output_feature_map=False): #256
        ...
        x = self.UpConv1(x)
        x = torch.cat([x, out2], dim=1)#######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.UnetConv1(x)
        x = self.upconv2_1(x, output_size=torch.Size([x.size(0),256,64,64]))
        x = self.upconv2_2(x)
        x = torch.cat([x, out1], dim=1)#######fill in here ####### hint : concatenation (Practice Lecture slides 6p)
