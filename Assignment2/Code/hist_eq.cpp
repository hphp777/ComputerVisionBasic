...
int main() {
	...
	fopen_s(&f_PDF, "hist_eq_PDF.txt", "w+");
	fopen_s(&f_equalized_PDF_gray, "hist_eq_equalized_PDF_gray.txt", "w+");
	fopen_s(&f_trans_func_eq, "hist_eq_trans_func_eq.txt", "w+");
	...
	//Drawing Histogram
	namedWindow("Grayscale_original", WINDOW_AUTOSIZE);
	imshow("Grayscale_original", drawHist_gray_PDF(input_gray));

	namedWindow("Grayscale_Equalized", WINDOW_AUTOSIZE);
	imshow("Grayscale_Equalized", drawHist_gray_PDF(equalized));
