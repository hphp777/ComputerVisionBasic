...
int main() {
	...
	//Fill the code/////////////////////////////////////
	cornerHarris(input_gray, output, 3, 3, 0.04, BORDER_DEFAULT);
	////////////////////////////////////////////////////
	...
	double minVal, maxVal;		Point minLoc, maxLoc; // 전부 값을 받아올 변수들
	minMaxLoc(output, &minVal, &maxVal, &minLoc, &maxLoc);

	int corner_num = 0;
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			//printf("%f\n", output_norm.at<float>(i, j));
...
Mat NonMaximum_Suppression(const Mat input, Mat corner_mat, int radius)
{
	...
	for (int i = radius; i < row+radius; i++) {
		for (int j = radius; j < col + radius; j++) {
			...
			//Fill the code///////////////////////////////////////////////////////////
			//We have to consider angle
			//여기서 input_mirror의 i,j 픽셀의 값을 받아 max_value로 놓기
			float max_value = input_mirror.at<float>(i, j);

			//index = 0 으로 초기 세팅	
			int index = 0;
			//-radious ~ radious까지 for문을 돌면서 (이중 for문으로) 
			for (int y = -radius; y < radius; y++) {
				for (int x = -radius; x < radius; x++) {
					//index를 1로 세팅하고 끝내기
					if (input_mirror.at<float>(i + y, j + x) > max_value) {
						index = 1;
						break;
					}
				}
			...
			//만약 위에서 for문을 통해서 나온 index가 1이면 corner_mat의 값을 0로 세팅하기.
			if (index == 1) {
				//corner_mat.at<float>(i, j) = 0;
