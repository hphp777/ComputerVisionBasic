		...
		for (int j = 0; j < col; j++) {
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					...
					float sum1_r = 0.0; //for Sx
					float sum1_g = 0.0;
					float sum1_b = 0.0;
					float sum2_r = 0.0; //for Sy
					float sum2_g = 0.0;
					float sum2_b = 0.0;
					for (int a = -n; a <= n; a++) { // for each kernel window
						for (int b = -n; b <= n; b++) {
							...
							sum1_r += (float)kernel_Sx.at<G>(1 + a, 1 + b) * (float)(input.at<C>(tempa, tempb)[0]);
							sum1_g += (float)kernel_Sx.at<G>(1 + a, 1 + b) * (float)(input.at<C>(tempa, tempb)[1]);
							sum1_b += (float)kernel_Sx.at<G>(1 + a, 1 + b) * (float)(input.at<C>(tempa, tempb)[2]);
							sum2_r += (float)kernel_Sy.at<G>(1 + a, 1 + b) * (float)(input.at<C>(tempa, tempb)[0]);
							sum2_g += (float)kernel_Sy.at<G>(1 + a, 1 + b) * (float)(input.at<C>(tempa, tempb)[1]);
							sum2_b += (float)kernel_Sy.at<G>(1 + a, 1 + b) * (float)(input.at<C>(tempa, tempb)[2]);
						}
					}
					output.at<C>(i, j)[0] = 0.3 * (G)sqrt(sum1_r + sum2_r);
					output.at<C>(i, j)[1] = 0.3 * (G)sqrt(sum1_g + sum2_g);
					output.at<C>(i, j)[2] = 0.3 * (G)sqrt(sum1_b + sum2_b);
