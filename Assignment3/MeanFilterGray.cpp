...
int main() {
	...
	output = meanfilter(input_gray, 5, "adjustkernel"); //Boundary process: zero-paddle, mirroring, adjustkernel
	...
	for (int i = 0; i < row; i++) { //for each pixel in the output
		for (int j = 0; j < col; j++) {
			...
			//밖으로 나간만큼 안으로 들어가네?
				...
				float sum1 = 0.0;
				float sum2 = 0.0;
				//zeropadding이랑 구현하는 방법 다동일한데 나누어주는것만 다르다.
				for (int a = -n; a <= n; a++) { // for each kernel window
					for (int b = -n; b <= n; b++) {
